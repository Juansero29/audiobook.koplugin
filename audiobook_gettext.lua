...
function GetText_mt.__index.changeLang(new_lang)
    ...
    if new_lang == "C" or new_lang == nil or new_lang == ""
       or new_lang:match("^en_US") == "en_US" or new_lang:match("^en$") then return end

    -- strip encoding suffix in locale like "zh_CN.utf8"
    new_lang = new_lang:sub(1, new_lang:find("%.") or (#new_lang + 1) - 1)
    -- strip territory variants after colon (rare)
    new_lang = new_lang:match("^([^:]+)") or new_lang

    -- Try full locale first (fr_FR), then language-only (fr). Neutral
    -- catalogs live under l10n/fr and l10n/es.
    local candidates = { new_lang }
    local lang_only = new_lang:match("^([A-Za-z][A-Za-z])")
    if lang_only and lang_only ~= new_lang then
        candidates[#candidates + 1] = lang_only
    end

    local po, file
    for _, cand in ipairs(candidates) do
        file = GetText.dirname .. "/" .. cand .. "/" .. GetText.textdomain .. ".po"
        po = io.open(file, "r")
        if po then
            new_lang = cand
            break
