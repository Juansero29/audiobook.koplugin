...
The overlay **Voice** menu on Android is a *language* request (`setLanguage("fr-FR")`) sent to the system engine. It does **not** download HuggingFace models into the plugin. With SherpaTTS as preferred engine and `fr-FR-miro-high` installed, French maps to Miro.
...
- List installed Android TTS voices by name (language picker already works)
